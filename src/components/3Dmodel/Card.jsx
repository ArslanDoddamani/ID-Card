...
function Band({ maxSpeed = 50, minSpeed = 10 }) {
  ...
  const isTablet = window.innerWidth >= 500 && window.innerWidth < 1025;
  const islaptop = window.innerWidth >= 1025 && window.innerWidth < 1400;
  ...
  useFrame((state, delta) => {
    ...
    if (rotate && !dragged && !hovered && card.current) {
      ...
      const increaseRotation = new THREE.Quaternion().setFromAxisAngle(
